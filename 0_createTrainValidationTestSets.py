...
csvfile=open(os.getcwd()+"/Classifications/Cleaned/Classifications_Vids123_filled_coded.csv", encoding="utf-8-sig",mode="r")
...
for key, value in dictionary.items():
    trainSetNumber=math.floor(float(len(value))*0.7)
    validationSetNumber = math.floor(float(len(value)) * 0.85)
    shuffledList=random.shuffle(value)

    trainSet=value[:trainSetNumber]
    validationSet=value[trainSetNumber:validationSetNumber]
    testSet=value[validationSetNumber:]
