bodyParts=["muscles","muscle",
"abs","ab",
'lower body',
"triceps","tricep",
"heels","heel",
"glutes","glute","gluts","glut",
'your back',"our back","our backs",
'upper body',
"arms","arm",
"feet","foot",
"knees","knee",
...
def addSilences(fullText, vidNum):
    ...
    while pTS < len(timestamps)-1 and pTrans < len(transcript)-1:
        ...
        if len(TSSilenceStart) > 0 and len(TSSilenceStop) > 0:
            if float(TSSilenceStop) - float(TSSilenceStart) > 2:
                newTranscript += "(silence) "
        dashWords = wordTS1.split("-")
        pTS += len(dashWords)
        pTrans += 1
...
silenceFullText=addSilences(fullText, 6)
colorTextHTML=getColoredHTMLText(fullText)
#fullTextWithTimestamp=addTimestampForKeywords(colorTextHTML, timestamps)
#fullTextWithTimestamp=addTimestampForEachWord(colorTextHTML, timestamps)
#fullTextWithTimestamp=addTimeStamps(allKeywords, colorTextHTML, 6)

# Copy and paste this long string into html file to make a webpage
#print(colorTextHTML)
print(silenceFullText)

# Output CSV with word counts
#myKeywordArray=getKeywordCounts()

# CHANGE THE FILENAME HERE
#makeKeywordCSV("/Users/jeans917/personal/sstp/aerobictextreviewLFS/Keyword Counts/video6_keywordCount.csv")