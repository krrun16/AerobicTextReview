...
equipment=["the weight","a weight","weights",
"dumbbell","dumbbells",
"box","boxes",
"chair","chairs",
"seat",
...
# function to add in time stamps
def addTimeStamps(originalWords,vidNum):
    newText = ""
    ...
    i = 0  # pointer for transcript
    j = 1  # pointer for time stamps transcript
    while i < len(originalWords) and j < len(timeStamps):
        transcriptWord = originalWords[i]
        timeStamp = timeStamps[j][1]
        hyphenSplitWords = transcriptWord.split("-")  # words with hyphens are divided in time stamps transcript csv
        newText += transcriptWord + " (" + str(timeStamp) + ") "
        i += 1
        j += len(hyphenSplitWords)
    return newText
...
def getKeywordCounts():
    ...
    for keywords in allKeywords:
        for i in range(len(keywords[0])):
            ...
            myKeywordArray.append([keyword,fullTextTimeStamps.count(keyword),keywords[2]])
...
# CHANGE THE FILENAME AND NUMBER HERE
...
fullTextNoTimeStamps = getFullText(txtFilename=txtFilename)
fullTextTimeStamps = addTimeStamps(textWords, videoNum)
colorTextHTML = getColoredHTMLText(fullTextTimeStamps)
...
#makeKeywordCSV(myKeywordArray,os.getcwd()+"/Keyword Counts/video5_keywordCount.csv")
